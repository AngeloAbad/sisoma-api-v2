...
import { LayoutDashboard, FileText, BarChart2, ClipboardList, Users, Database, Bell, Settings, Plus, Edit3, Eye, Trash2, Download, Search, ArrowLeft, X, Mail, Zap, EyeOff } from 'lucide-react'
      ...
      {loading ? <Loading/> : forms.length===0 ? <Empty Icon={FileText} title="Sin formularios" sub="Crea el primer formulario SSOMA." action={<PriBtn onClick={()=>{setEditForm({});setSec('builder')}}>Crear formulario</PriBtn>}/> : (
        <div style={{ ...card({ overflow:'hidden', padding:0 }) }}>
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            <thead><THead cols={['Nombre del formulario','Categoría','Preguntas','Respuestas','Estado','Acciones']}/></thead>
            <tbody>
              {forms.map(f=>(
