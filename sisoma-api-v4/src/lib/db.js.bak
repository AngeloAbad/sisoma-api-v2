  ...
  async login(identifier, password) {
    if (isConfigured) {
      const email = identifier.includes('@') ? identifier
        : (() => { const u = LS.get('users', []).find(x => x.emp.toUpperCase() === identifier.toUpperCase()); return u?.email })()
      if (!email) return { error: 'No. de empleado no encontrado.' }
  ...
  async register(form) {
    if (isConfigured) {
      const { data, error } = await supabase.auth.signUp({
        ...
        options: { data: { emp: form.emp, name: form.name, role: form.role, area: form.area, empresa: form.empresa } }
      })
      if (error) return { error: error.message }
      return { data: { message: 'Revisa tu correo para confirmar tu cuenta.' } }
    }
    const users = LS.get('users', [])
...
export const db = {
  ...
  async createForm(formData, questionsData) {
    if (isConfigured) {
      const { data: form, error: fe } = await supabase.from('forms').insert([formData]).select().single()
      if (fe) return { error: fe.message }
      if (questionsData?.length) {
        const qs = questionsData.map((q, i) => ({ ...q, form_id: form.id, sort_order: i + 1 }))
  ...
  async updateForm(id, formData, questionsData) {
    if (isConfigured) {
      const { error: fe } = await supabase.from('forms').update(formData).eq('id', id)
      if (fe) return { error: fe.message }
      if (questionsData) {
        await supabase.from('questions').delete().eq('form_id', id)
        if (questionsData.length) {
          const qs = questionsData.map((q,i) => ({ ...q, form_id: id, sort_order: i+1 }))
          await supabase.from('questions').insert(qs)
        }
      }
      return { data: true }
  ...
  async submitResponse(responseData, answersData) {
    if (isConfigured) {
      const { data: resp, error: re } = await supabase.from('responses').insert([responseData]).select().single()
      if (re) return { error: re.message }
